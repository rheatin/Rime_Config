...
function M.func(input, seg, env)
    -- 日期
    if (input == M.date or input == "date") then
        ...
        -- 时间
    elseif (input == M.time or input == "time") then
        local current_time = os.time()
        ...
        -- 星期
    elseif (input == M.week or input == "week") then
        local current_time = os.time()
    ...
    elseif (input == M.datetime or input == "datetime") then
        local current_time = os.time()
        ...
        -- 时间戳（十位数/十三位数，到秒及毫秒，示例 1650861664）
    elseif (input == M.timestamp or input == "ts" or input == "now") then
        local current_time = os.time()
        yield_cand(seg, string.format('%d', current_time))
        yield_cand(seg, string.format('%d000', current_time))
